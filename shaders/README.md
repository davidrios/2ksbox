...
- `syncmaster-753dfx.slangp` is the reference rig's monitor (doc 09), a
  ...
  from the tube's geometry and are not yet calibrated against photographs
  of it. The file says where each value came from.
...
parameter overrides. Each profile is one file under the data directory's
...
`--shader` / `--shader-params` at launch. A hand-written `machine.toml` can name a
