...
| `tools/zc-vulkan-test.c` | the same ring with the **frontend's Vulkan import** and nothing else — no guest, no player, no wgpu (doc 12 §4). `--stage=` picks how much of `player/src/dmabuf.rs` to do (`none` the control, through `mem`, `image`, `nodedicated`, `linear` to `full`), `--use=copy` reads every frame back through Vulkan as well and `--use=shader` leaves the image in the layout wgpu leaves a sampled texture in; each slot's memory is checked with the CPU after every frame. All stages clean, which is what rules the import description out. Local only (Linux, Vulkan), not in `scripts/test.sh` |
