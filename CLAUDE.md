...
- **A real Voodoo 2 is emulated too, beside the pass-through** (doc 21,
  ...
  emulation — an **SLI pair** since 2026-09-18 (user decision, doc 21
  §12), two boards on function 0 and function 1 of its slot, which is the
  only way to 1024x768; `sli=off` is the single board, and the guest asks
  for the interleaving (`fbiInit1` bit 23) as it would on the cable. The
  **screen filter** is a pick too (`filter=off|v2|4x1|2x2`, doc 21 §13,
  patch 72): the card's own dither removal, or the Voodoo 3's two, ported
  from 86Box's Banshee sources into `voodoo/voodoo_vbfilter.c` because
  those files are not vendored — the emulation itself is
  vendored **verbatim** under `voodoo/86box/` (never edit those
