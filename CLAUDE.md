...
- Win98 and XP both run `-vga none -device d3dpt-vga` with our driver
  ...
  `cirrus` on the new Other family, and — since 2026-09-09 — `cirrus`
  (still the default) or `std` on DOS, the one family where the adapter
  is not a driver question but *which VESA BIOS the title finds*, and
  where changing it installs nothing. The **defaults are
