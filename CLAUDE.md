...
- **A real Voodoo 2 is emulated too, beside the pass-through** (doc 21,
  ...
  `GLIDE3X.DLL` in another process for ~3 s under TCG, so a Glide program
  started at the desktop wedges — wait a few seconds; the device warns
  `the card is being re-initialised` when it happens.
