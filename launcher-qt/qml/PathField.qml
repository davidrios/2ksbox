...
// mount the picker can't reach). `FileDialog` is Qt's own, which is the
// desktop's: NSOpenPanel, IFileDialog, and on Linux whatever the
// platform theme offers. `main.rs` asks for the XDG desktop portal's
// there, because a session Qt matches no theme to gets Qt's own picker.
import QtQuick
...
RowLayout {
    ...
    /// property), and the field would keep showing the last path it was
    /// given while the model moved on. That was "New profile… still
    /// shows the previous preset": the shader editor's field had been
    ...
    /// The same path, but only when it came from the dialog rather than
    /// the keyboard, for a field where choosing a file is the whole
    /// answer (the disc shelf's adder acts on it at once). `edited` is
    /// emitted for it too, so a field that doesn't care about the
    /// distinction can ignore this one.
    ...
    /// including the directory the next empty field's dialog opens in.
    ...
    /// Stripping `file://` off `url.toString()` leaves `[` and `]` as
    /// `%5B`/`%5D`, so a disc called "Game [1996]" went on the shelf under
    /// a path that does not exist and the machine booting it would not
    /// start. `fileUrl` is what a probe hands this in place of a dialog.
    ...
    /// What the text field is showing. The probes read it: `value` is
    /// what the owner meant the field to show, and the two disagreeing
    /// is the bug this field is shaped to prevent.
    readonly property alias shownText: field.text
    /// What the dialog is handed, for the same probes: a filter
    ...
    /// dialog closed the window and the sheet with it (seen on the disc
    /// shelf).
    ...
    Button {
        ...
        // dialog (any field's) was browsing, which changes between one
    ...
    FileDialog {
        ...
        // Every glob comes in both cases (`browse::extensions`), which
        // makes the filter combo's text twice as long as it needs to be.
