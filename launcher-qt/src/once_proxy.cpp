...
//
// `tools/qtmin/README.md` has the full diagnosis and a reproduction. In
// short: cxx-qt's generated crate initialiser calls `std::call_once` from
// a static initialiser. mingw's `call_once` parks the callable in
// `std::__once_call`, a `__thread` variable reached through **emulated
// TLS**, and hands `pthread_once` the address of `__once_proxy` to run.
// `__once_proxy` lives in `libstdc++-6.dll`, and emutls keys its storage
// per libgcc. There are two here: the one linked statically into this
...
// this module's emutls, the same registry `call_once` wrote it to. It is
// the whole of libstdc++'s own implementation (`src/c++11/mutex.cc`):
...
// callable through `std::__get_once_call()`, a function in the DLL, so
// the same module writes and reads it and the bug above cannot happen.
// There `std::__once_call` is not exported at all, so this definition
// would not link ("undefined reference to std::__once_call", MSYS2's
// GCC 16). Fedora's GCC 15, which builds the package, has the extern
