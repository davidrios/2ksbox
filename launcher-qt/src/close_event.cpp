...
// button goes the other way: the platform (Cocoa's `windowShouldClose`, a
// WM_CLOSE, an X11 `WM_DELETE_WINDOW`) hands Qt a close event with no
// such mark. Only the second can re-enter `close()` from a
// `visibleChanged` handler, which left the main window locked behind a
// dismissed dialog on macOS (`qml/Main.qml`, `closeIfShown`). So the
// probe sends the event rather than calling the method: the same
// `QCloseEvent` `QWindowSystemInterface::handleCloseEvent` sends,
// delivered through the public `sendEvent` rather than the QPA private
// header.
//
