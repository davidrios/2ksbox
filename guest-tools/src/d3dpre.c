/*
 * d3dpre.c — D3DPRE.EXE: point this session's DirectDraw at WineD3D when
 * the host has no Direct3D of its own, and at Windows' own when it has
 * (doc 19 §43; Windows 98/Me only).
 *
 * The problem it solves. On 9x the WineD3D copy of `DDRAW.DLL` next to a
 * game reaches that game only if it is the first program of the session to
 * touch DirectDraw: once Windows' own `ddraw.dll` is loaded — and
 * `DDHELP.EXE` keeps it loaded for the rest of the session — every later
 ...
 *   "DDRAW"="ddrawme.dll"
 *
 * and the loader reads that per `LoadLibrary`, not once at boot — a value
 * written at login is in force for every program started afterwards, with no
 * restart (measured 2026-09-19). Nothing replaces a protected system file,
...
static void gl_restore(void)
{
    ...
    if (CopyFileA(ours, sys, FALSE)) {
        ...
        logp("  OPENGL32.DLL is not ours (%lu bytes) and is in use — WineD3D will be slow",
             (unsigned long)sb);
    }
}
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmd, int show)
{
    ...
    if (want_wine && !have_ddraw) {
        ...
        logp("d3dpre: %s is not in the system folder — nothing to switch to",
             DDRAW_TO);
        want_wine = FALSE;
    }
