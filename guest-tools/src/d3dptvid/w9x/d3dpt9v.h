...
/* The four the main VDD calls around a screen switch, when a full-screen
 * DOS box takes the adapter to VGA and gives it back. Repeated from
 * minivdd.h's function list because the VxD builds against vmm.h alone. */
#define VDD_PRE_HIRES_TO_VGA        4
#define VDD_POST_HIRES_TO_VGA       5
#define VDD_PRE_VGA_TO_HIRES        6
#define VDD_POST_VGA_TO_HIRES       7

/* The main VDD is about to draw a message screen (a blue screen) in a VGA
 * text mode it programs itself, without a VM switch. A fatal exception in
 * the Windows VM goes through the four above instead (doc 19 §29). */
#define VDD_SAVE_MESSAGE_MODE_STATE 45

/* The notifications the VxD logs (the first few of each), so the order of
 * a screen switch, a mode change or a blue screen can be read off the log
 * rather than guessed from the DDK. All of them save every register and
 * return nothing, so answering them changes nothing. */
 ...
 * above (minivdd.h's VDD_REGISTER_DISPLAY_DRIVER_INFO), defined here so
 ...
 *
 * The ring-3 HAL DLL needs the linear addresses. It is 32-bit and cannot
 * use a selector, and DPMI cannot translate these ones because they are
 * GDT selectors the mini-VDD built and DPMI's "get segment base" only
 * knows the LDT. Only the VxD knows both, so it returns both. */
#define D3DPT_VDD_REGISTER_INFO 0x83

/* The selectors are DPL 3 because the display driver is ring-3 code and
 * the DIB Engine draws through them. */
