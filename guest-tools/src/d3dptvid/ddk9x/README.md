...
against. They do for `w9x/` what `../ddk/` does for the XP driver, for
the same reason. **The Microsoft DDK is not used here.**
...
| `ddrawi.h`, `dmemmgr.h` | the DirectDraw driver interface the 16-bit half publishes its HAL through, with the `DDHALINFO` it builds, the `DCICMD` escape it answers, the `DD32BITDRIVERDATA` naming the ring-3 DLL (doc 19 §2) |
