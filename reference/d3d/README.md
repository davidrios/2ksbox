...
- **The shader golden has no pixel shader.** The rig's d3dx9_36 HLSL
  ...
  this build, since an emulated run must draw what the rig drew, so the
  ps_1_1 refusal stays until a new golden set is taken. With the HUD masked the
...
| d3dgame8 windowed (`D3DSWAPEFFECT_COPY_VSYNC`) | 43–44. The GeForce driver paces a windowed COPY_VSYNC present at half the refresh rate. That is real behaviour; remember it when reading d3dgame8 windowed numbers |
