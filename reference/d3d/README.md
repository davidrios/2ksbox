...
| `d3dgame9-w300-vs11.bmp` | `D3DGAME9 -shader -frames 600 -dump 300 d3d9_dump_shader.bmp` | the cubes through the vs_1_1 vertex shader with the **fixed-function pixel stage** (below) |
...
- **The shader golden has no pixel shader.** The rig's d3dx9_36 HLSL
  ...
  That build's log wrongly calls the case "fixed function", because
  `draw_cubes` keys on the vertex shader alone. An emulated run must draw
  what the rig drew, so rendering stays frozen at this build, ps_1_1
  refusal included, until a new golden set is taken. With the HUD masked
  the two goldens differ in 9.7 % of pixels, all of them the cubes.
