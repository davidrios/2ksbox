...
| `04-wsi-headless` | a window-less WSI driver (`src/wsi/headless`, `DXVK_WSI_DRIVER=Headless`): one fake monitor with a D3D9-era mode list, no window is ever valid, `createSurface` fails. With a NULL device window DXVK's d3d9 swapchain creates no presenter and `Present` is a no-op, so the paravirtual device's executor (`d3dpt/exec`) runs entirely off-screen and reads the backbuffer back. Since 2026-09-07 it is the **only** WSI we build: the patch also drops upstream's "SDL3, SDL2, or GLFW are required to build dxvk-native", `configure-dxvk.sh` passes `-Dnative_sdl2=disabled`, and `tools/dxvk-d3d9-test.cpp` and the native oracles have no window path left | never: upstream has no headless WSI |
...
only `DXVK_WSI_DRIVER=Headless` set (2026-09-03 with SDL2; window-less
since 2026-09-07). The native library dlopens the Vulkan loader by bare
name, so on macOS run with `DYLD_LIBRARY_PATH=/opt/homebrew/lib
VK_ICD_FILENAMES=<icd> DXVK_WSI_DRIVER=Headless`. On MoltenVK 1.4.2 the harness ends with DXVK's
...
WSI works on KosmicKrisp too: it wrote a BMP identical to the SDL2 path's
(384 vs 106 fps for the 60-frame triangle) — which is what made dropping the
SDL2 path outright safe.
