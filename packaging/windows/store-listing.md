...
least one, up to ten, no added text or logos. The player's own
screenshot (Ctrl+Alt+S, `2ksbox-NNNN.png` in the working directory, or
`PLAYER_SHOT_DIR`) is the guest's frame at the guest's resolution, so
`scripts/store-screenshots.ps1` scales each by the smallest whole factor
that reaches the minimum, with no smoothing, into
`build/win/package/store/`. A good set:
