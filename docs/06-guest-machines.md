...
era OS that is none of those (BeOS, a period Linux, OS/2), with standard
hardware and nothing of ours. The defaults live in
`launcher-core/src/bundle.rs` (`video_choices`, `sound_choices`,
`music_choices`, `pad_choices`, `default_*`), and every choice list's
first entry is the family's default. The machine form is doc 07, the
sound and music devices doc 20, the display drivers docs 15 and 19, the
Voodoo 2 doc 21.

**Common to all four.** i440FX + PIIX (`-machine pc`), `-cpu pentium3`
(with the form's optimization switches as its properties), IDE disk
(qcow2) and our ATAPI CD (doc 17). There is **no network card on a new
machine**, nor on a bundle that does not say, so an unpatched guest is
not put on a network before anyone asks (`-nic none`, since QEMU
otherwise adds an e1000 of its own). There is **no gamepad** until one is
picked (`bundle::Pad`, M13), and a Voodoo 2 checkbox, off
(`voodoo2,addr=0x05`). Every display adapter lands at PCI `0x02` and the
NIC and sound card are pinned below it (`0x03`, `0x04`), so turning
networking off or changing the adapter never slides a card into
another's slot. A card that moves is a hardware change an installed guest
re-detects.
...
| Video | `d3dpt-vga` + our driver (`cirrus`) | ours is the whole display path (doc 19): mode table, desktop from VRAM, paced page flips, Direct3D through the driver. Default by user decision; the Cirrus is Windows' in-box driver and the A/B |
...
it through DirectInput **and** winmm's `joyGetPosEx` over VJOYD (the
`pad-guest-98` check measures both). "Standard Game Port" buys a Windows
game nothing, so M13 does not install it. The gameport at 0x201 is for a
DOS box under 98 and 1995 titles. It is not Plug and Play (Add New
Hardware, then calibrate), and the form says so.
...
- TCG has faulted RUNDLL32 in Display Properties since 7.2 (issue 1964).
  It is cosmetic, the OS survives, and KVM/WHPX are unaffected.
...
ISO (`SETUP.EXE`, `guest-tools/README.md`). A known quirk still to
document is DOS-compatibility-mode storage regressions.
...
98 SE CD comes out ACPI (user-confirmed). An image installed before the
stamp is repaired through Device Manager (PnP-BIOS → PCI Bus,
...
| Component | Default (alternatives) | Why |
|---|---|---|
| CPU model | `pentium3` | deliberately unchanged, since what makes a machine feel like a 486 is the rate, not the CPUID string |
...
| Gamepad | none (gameport at 0x201, key mapping) | no USB stack. A DOS game reads the port by arming four one-shots and counting, so the count depends on the guest's speed. That is the other reason this family is paced: unpaced, an untouched axis wanders by half |
...
(2.4 → 3.5 s) because booting is mostly waiting.
...
(user decision). DOS games run on pure DOS, either the DOS family or
98's "Restart in MS-DOS mode", where `TESTS\QCLOCK.COM` reads 100 %
...
up to 153 ticks, the "momentary speed-up".) Left untried on purpose are
measuring the VM's tick lag, `[386Enh] TrapTimerPorts=Off`, and the rate
VTD programs the PIT to.

## Other

For an era OS that is neither Windows nor DOS. By design nothing is
tested against a specific guest, so every choice is the one likeliest to
have had an in-box driver on a nineties system.
...
| Video | `std` (`cirrus`) | the Bochs VGA with VBE 2.0 and a linear frame buffer, which a period VESA driver wants and modern Linux's `bochs-drm` binds. The Cirrus is a real chip, so an era guest may have a *native* driver (BeOS R5, XFree86). Not `d3dpt-vga`: its driver is Windows-only |
...
On Windows the whole display path is built on ours. The Cirrus is for a
machine whose driver is not installed yet, for A/B-ing a title that
...
- **Changing it under an installed Windows or Other guest is a hardware
  ...
  only when the adapter was actually changed (`Form::video_warning`).
  **DOS is told something else.** The machine just boots, and only a
  game's own setup, which may
...
- qcow2 with named snapshots ("fresh install", "drivers installed",
  "pre-game-X") in the launcher, because the retro workflow is
  reinstall-heavy.
