...
The track was merged to `main` on 2026-09-04; its branch is deleted. It is
validated bit-exact on aarch64 (the Air) and x86-64. This record keeps the
...
- **Editing a patch.** Follow the recipe in `patches/qemu/README.md`. Patch
  11 is not the last patch to touch its TCG files, because patch 12 edits
  the same files after it. So a change to 11 means regenerating 12 as well.

## What stayed open

- **A real-workload number.** No one has yet run a Direct3D title in XP
  with and without `x87-fast=off,sse-fast=off`. That run would give the
  first end-to-end number for patches 06, 11 and 12 together. M4 lists the
  same item.
...
- **Still on helpers:**
  ...
  - `cvtpi2ps`/`cvtps2pi`;
  - `pmuludq`;
  - SSE3 `haddps`/`addsubps`, SSSE3 and VEX forms. These have no
    relevance to the era.
- **The x87 C transform** (16–18 % of the rig). This is limited by the
  shadow translator's throughput on cheap ops. Improving it is a doc 13
