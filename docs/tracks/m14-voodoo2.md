...
The emulated 3dfx Voodoo 2 is 86Box's emulation as a QEMU PCI device,
`-device voodoo2`. It sits beside qemu-3dfx's Glide and OpenGL
pass-through and never replaces it. Doc 21 is the design and holds
everything learned: why a chip, the shim, threads and timers, the
display path, the command FIFO in RAM, the login helper, the stranded
client, the dither and the ring's ordering. This file is the track's
working state: scope, test loop, traps and what is open. ADR-016 is the
decision, and `patches/openglide/README.md` "Emulating the chip
instead" is the argument it was opened on. The track runs on `main`.
...
- The machine form's "Emulated 3dfx Voodoo 2" checkbox (`voodoo2` in the
  bundle, `-device voodoo2,addr=0x05`), shared with M6 and edited
  minimally.
...
- **3dfx's own Win98 driver and Glide 2 and 3 run on it.** By hand on
  ...
  3dfx build (races, since `texmem=2`), and 3DMark 99 on the Windows
  PC. DxDiag's DirectDraw test passes full screen. A second game after one
...
`EXTRA='-device voodoo2,addr=0x05'`, the launcher's slot. Any other
slot is new hardware to Windows and brings a restart prompt before the
shell. The harness waits
...
The device writes `voodoo2:` lines on QEMU's stderr: the
...
- Porsche's and other menus cannot be clicked headless while the card has
  the monitor, because `relclick` steers by `d3dpt-vga`'s cursor registers, which
  do not move then.
...
- One TCG guest at a time, and end scripted Win98 runs with the ACPI
  power button (`docs/00-status.md` "Gotchas").
...
1. **Left-over state between Glide windows.** A second game after one has
   ...
   since the ring and teardown fixes). Trace it with
...
3. **DxDiag's Direct3D 7 test fails at step 46, `GetDC`**
   (`DDERR_CANTCREATEDC`) on the card. Unknown: whether a real Voodoo 2
   with this driver fails it too (the card has no GDI) or the device
   lacks something.
