...
branch is deleted. This doc keeps the scope, the test loop, the traps
specific to this code, and what stayed open.

The design and every finding live elsewhere:

- Doc 05 has the problem statement and the acceptance table, one row per
  protection.
- Doc 17 is the spec:
...
- `docs/tracks/m5-dirdisc.md` is M5g, a host folder as a disc.
- Doc 07 has the disc shelf (patch 52, M6).
...
- **Watch a check fail before believing it passes.** Three protections
  "passed" before any had been seen to refuse a disc; the clue was that
  the `discx repair` copies passed too. The trace showed that SafeDisc 2
...
- **Put a protected title's disc on its own IDE channel** (`ide.1`, as the
  ...
  disk's slave.
- **Check which device sent a sense reply before calling it a bug.**
...
- **Re-measure the protected dumps on the rig** (also a
  ...
  §2.5), check that every L-EC failure on FIFA 2002, AoM
...
- **Speed model.** The drive delivers as fast as the host reads, and QEMU's
  ...
  paces itself on CD reads minds needs a `speed=` property on `ide-cd`
  that both drivers honour (`docs/00-status.md` has the measurements).
- **Not recorded:**
