...
floor: the **same** paravirtual device and the **same** executor as
everywhere else, run on **Wine's d3d9 (WineD3D over OpenGL) in a Wine
process on the host**, instead of a 2015 Wine copied into the guest next
to every game. Opened 2026-09-22 on the user's decision ("running an old
...
is the API carried across. The Windows host's equivalent is not Wine but
the system's own d3d9 in process (ADR-007's second amendment).
...
- The shared file behind the devices' memory: `d3dpt/hw/d3dpt_vga.c`
  (VRAM, QEMU patch 73) and `d3dpt/hw/d3dpt_mm.c` (the SysBus window).
- Build and packages: the `--wine` stage of
  `scripts/build-d3dpt-exec.sh` (the PE pair into `build/d3dpt/wine/`),
  `scripts/build.sh`, `package-linux.sh`, `package-macos.sh
  --community`, the Flatpak manifest.
- The launcher: `launcher-core/src/host_gpu.rs` (`D3dBackend::Wine`, the
  third verdict), the form's Direct3D note (`wizard::Form::d3d9_note()`),
...
- Tests: the `exec-wine` check; the `EXEC=` knob of
  ...
  `tools/tcg-profile.sh`; `tools/macvm-wine-spike.sh`,
  `tools/macos-wine-spike-local.sh`.
- Shared, edited minimally: `d3dpt/exec/d3dpt_exec.cpp`, which needs
  nothing for Wine — the hidden window and executor-owned scene of the
  `system` branch are what wined3d wants. `d3dpt/d3dpt_proto.h` is
  untouched.
- **Not this track's until step 6:** everything of WineD3D-in-guest —
  the wine9x build in `guest-tools/build-wrappers.sh`, `patches/wine9x/`,
...
Built and working end to end on the M1 Air; waiting on one acceptance
run.
...
- **XP guest**: D3DGAME8 within the rig budget (600 frames in 3.4 s
  against 2.1 s in process), the ten DX8 DDI probes with the in-process
  verdicts (nine PASS, PATCHTST NOT OFFERED), and FIFA 2000 playing a
  match at 22.6 frames/s against 19.0 in process.
- **Win98 guest**: D3D7TEST through the 9x HAL, 300 frames at 57 fps,
  byte-identical to `dp2-test.bmp`.
- **Launcher and packages**: the third verdict, `--host-check` exit 0
  through Wine, `--paths` / `--companions` listing the Wine and the pair;
...
ADR-018 gives the first three; so they are not re-proposed:

- **wined3d in process (winelib).** `d3d9.dll` and `wined3d.dll` are PE
  modules over `opengl32.dll`, needing Wine's loader and a `wineserver`;
  there is no `libwined3d.so`, and a winelib binary *is* a Wine process.
- **An executor of our own over GL, Metal or wgpu**: a second
...
with macOS 15, run from a Terminal booted into it — one machine runs
one macOS at a time — with `tools/macos-wine-spike-local.sh` from the
other volume's checkout. A bare macOS 15 needs Rosetta
(`softwareupdate --install-rosetta --agree-to-license`); its Python is
the Command Line Tools stub, so the script falls back to uv's.

**Which Wine**: Linux uses the distro's (`wine`/`wine64` on `PATH`).
On a Mac, Homebrew's Wine casks are disabled (not notarized), so it is
WineHQ's tarball from Gcenx's releases (x86_64, under Rosetta on Apple
Silicon) — unpacked into `build/wine/` in a checkout, or an app in
`/Applications`, which is where a packaged app looks (besides `PATH` and
`D3DPT_WINE`). Native arm64 Wine gets no OpenGL in `winemac.drv`.

## Traps

- **Run `install` on an XP overlay first.** `winxp-m7.qcow2`'s driver
  (2026-09-08) predates the any-newer-version rule and refuses the
  adapter: XP runs on its VGA fallback, D3DGAME8 dies in a modal box, and
  the killed guest loses its unflushed logs. The proof of an install is
  the mode-set line after the restart.
- **XP with its display driver refused shows a black screen** on the
  adapter — the VGA core in a chained 256-colour 800×600 mode
  (`sr4=0a gr5=50`) that renders nothing — while BIOS text and mode 13h
  render. Every screendump of a failed install run is black; read COM1.
- **A first batch stalls the vCPU for as long as wined3d compiles its
  shaders** (the first second at ~6 fps). The child therefore makes its
  device at the library's probe, during the adapter's realize, not at the
  first `create()` inside an MMIO read — a 3.5 s stall there reset the
  Win98 machine. A long stall also let patch 65 raise a quarter second of
  owed PIT ticks back to back, which Windows 98's early-acknowledging
  timer handler nested into a VMM fault; the patch now paces the
  catch-up (`patches/qemu/README.md`, 65). Any executor that slow would
  have done it — which is what a below-floor host is.
- **Carry `base98-us`'s Voodoo 2 in the launcher's slot** in a harness
  run: without the card 3dfx's login helper fails in an "Error" box that
  takes exclusive mode from the test (`DDERR_NOEXCLUSIVEMODE` at the first
  `BeginScene`); in another slot Windows finds new hardware and wants a
  restart before the shell.
...
- **The shared-file path is POSIX-only** in QEMU:
  ...
  undeclared function"). Windows loses nothing — it has no remote
...
- Homebrew's mingw links `libwinpthread-1.dll` dynamically (the Fedora
  cross image's does not); the pair needs it beside the `.exe`.
...
1. **The spike** — done: the Windows executor on Wine's own d3d9, both
   host tests byte-identical to DXVK, no executor change. Left: the same
   on Linux Wine with a real GL (the rig), which `scripts/test.sh host`'s
   `exec-wine` does given the distro's Wine — a run, not a question.
2. **The transport** — done (doc 14). A pipe round trip per `submit` did
   ...
   process). If a guest profile ever shows it, the answer designed for
   it is a bounded spin on a sequence word in the shared file before
   blocking on the pipe.
3. **The guest** — done on XP and Win98 (State). Open beside it: the XP
...
5. **A real game on a below-floor host** — the acceptance, and what is
   left: the community app on a real macOS 15. It is built on the 26
   side (`scripts/package-macos.sh --community --no-sign --no-dmg --out
...
6. **Retire WineD3D-in-guest, in one commit**, once 5 passes: the ISO's
   ...
   `patches/wine9x/`, `tools/xp-wined3d-test.sh` and
   `wined3d-sys-test.sh`, the launcher's WineD3D advice, doc 04's rows,
   doc 19 §42–44's status, and CLAUDE.md's "never propose deleting the
   `WINED3D\` ISO folder" sentence. The Flatpak's Wine (bundle a trimmed
   one, the `org.winehq.Wine` extension, or "install it") and its PE pair
   (the SDK has no mingw: a checked-in build, as `firmware/vgabios-*.bin`,
   or a release asset) are decided here too.
