...
| `shader-defaults` | the first-run shader offer without a toolkit, and that a new machine starts on the CRT Aperture profile once the library has it and on the app default before |
...
| `scripts/store-screenshots.ps1 <png>...` | the player's Ctrl+Alt+S shots scaled by the smallest whole factor to the Store's 1366×768 minimum, nearest-neighbour, into `build/win/package/store/`; refuses one that would pass 3840×2160 |
