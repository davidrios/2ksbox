...
| `scripts/win-sideload.ps1 [<msix>] [-NoInstall\|-Remove]` | on the PC: certificate, trust (one UAC prompt), sign, install, then the installed launcher's `--diagnose` run with package identity must write its `library … (packaged)` line into `%USERPROFILE%\2ksbox\launcher.log`; the real answer to "where does a Store install keep the machines" |
