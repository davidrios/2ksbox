...
through the `qemu-embed` crate's **hand-written** bindings (the API is
...
- **Display.** After init, with the BQL held, a `DisplayChangeListener`
  ...
  so never retain it, `ui/console.c:853`); `dpy_gfx_update` a clamped
  dirty rect; `dpy_refresh` is where we call `graphic_hw_update()`, the pull
...
That gives full QMP including events, the monitor on its own iothread,
no filesystem path and no network. (A true pipe would be
...
2. **`20-embed-audio.patch`**, the `embed` audiodev. The driver lives in
   ...
   audio_create_pdos()`**. Without it the pdo is NULL and
...
it (doc 07). The file's header has the whole argument. The rules:
...
Two earlier designs failed, and they explain this one. One topped the
ring up to a
target every tick and dropping whole ticks over it (12 clicks in 20 s
against a 2048-frame device, 922 against 4096). The other was a 10 ms
version that never caught up after a stall (XP audio grew laggier the
longer it ran).
...
- `qemu_cleanup` is incomplete (`runstate.c:929` TODO), so it is **one VM
  ...
  02). Never exit the process while the QEMU thread is alive, because
  QEMU's atexit handlers race `qemu_cleanup`. The player joins it and
...
once 3D frames stop and the guest drew on it). 3D frames arrive as a
copy or a ring slot index (`dmabuf.rs`, `iosurface.rs`). Keyboard and
mouse go from winit through `qemu_embed_key` / `mouse_*` and one
`input_flush` per batch; the pad through `pad_state` once per published
frame. The rest of the player is doc 03 and doc 07.
