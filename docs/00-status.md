...
| **M16** DirectX 9 driver, no custom DLLs | `tracks/m16-dx9-ddi.md` | the DX9 DDI in `guest-tools/src/d3dptvid/` (with M10 / M7's files), its decoder cases in `d3dpt/exec/d3dpt_exec_ddi.cpp`, the Wine test suites' build, runner and baselines, doc 15's DX9 section | Opened 2026-09-25 (ADR-021); step 0's suites run on XP (`xp-driver-test.sh winetest`) · the rig's baselines, the crashes the first run found, step 1 |
