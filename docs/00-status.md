...
| **M16** DirectX 9 driver, no custom DLLs | `tracks/m16-dx9-ddi.md` | the DX9 DDI in `guest-tools/src/d3dptvid/` (with M10 / M7's files), its decoder cases in `d3dpt/exec/d3dpt_exec_ddi.cpp`, the Wine test suites' build, runner and baselines, doc 15's DX9 section | Opened 2026-09-25 (ADR-021). Step 0's suites run on XP; step 1 done: XP's `d3d9.dll` takes the driver as a DX9 / SM3 device (protocol v14) and renders D3DGAME9 and D3DFEAT9 identical to the native frames (blits, queries, DX9 vs 1.1) · mip generation, instancing, the float formats (step 3), the rig's baselines |
...
| Direct3D executor (doc 14) | Protocol v14, one decoder, four D3D9s. DXVK (native and on Windows) is the default and the golden reference. Below the Vulkan 1.3 floor: Windows' own `d3d9.dll` (`D3DPT_D3D9`, ADR-007's second amendment) or Wine's on a Linux / macOS host (`exec=wine`, ADR-018, M15). `no-exec=on` models a host with no executor. |
| XP display driver (doc 15) | `d3dpt-vga`, register set v5. DirectDraw, a DirectX 9 DDI with shader model 3.0 to `d3d9.dll` (M16, 2026-09-25: the face and the first tokens) and a DirectX 8 DDI with hardware T&L, shaders 1.x, palettes, colour keys, VRAM buffers, 16 streams, cube / volume textures, MSAA, gamma. FIFA 2000, Max Payne, Vice City, Moto Racer and Diablo play. |
