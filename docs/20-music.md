...
fixes to QEMU's SB16 that the same work turned up. Each family's audio
hardware is in doc 06, the track's state in `docs/tracks/m12-music.md`,
and every test tool in `docs/testing.md`.
...
`libqemu-embed-i386` alike and driven by two device models, as `libdisc`
is (doc 17 §5.2, patch 50). We rejected a synthesizer in the player, fed
MIDI bytes through the embed API, for three reasons:
...
3. **No new libraries.** All three engines are pure Rust (`nuked-opl3`,
   ...
   add to the libraries the macOS app and the Flatpak carry.

A **host MIDI port** (to a real MT-32, an SC-55 or the OS's own synth) is
deliberately left out. It is a host device, so it belongs on the
...
|---|---|---|---|
| OPL3 | `nuked-opl3` | LGPL-2.1+ | The Nuked core is a register-level model of the YMF262 from a die scan, and every serious DOS emulator has moved to it. Its `Opl3Device` layer carries the two timers and the status register, which QEMU's `adlib` lacks and every AdLib *detection* routine reads. |
...
realize. Without that check a stale staticlib is a guest that plays
...
audio timer in the main loop), and the BQL is the serialization. Nothing
blocks or allocates on the render path, and every entry point catches a
panic. A synthesizer that falls over costs the guest its music, never
QEMU its process.
...
the bank decides the sound far more than the engine does. One ships,
`soundfonts/TimGM6mb.sf2` (5.7 MB, GPL-2, the bank MuseScore 1.x shipped
and Debian packages). `soundfonts/README.md` has its provenance and hash,
and says why we don't ship the better-sounding GeneralUser GS: its author
does not vouch for every sample's origin, which is fine for a bank a user
picks but not for one in a GPL package. The machine form says any `.sf2`
can be picked instead.

**Where the bank comes from is not the bundle's business.** A machine
saying `synth=gm` is the normal case. The device finds the file by the
rule every companion of ours uses (the Glide wrapper's, patch 33): the
`soundfont=` property, then `LIBSYNTH_SF2` (a packaged player sets it to
its own copy in `player/src/companions.rs`), then
...
with `sbbase=`, the mirror a Sound Blaster has at **2x0–2x3**. An
SB-aware game looks there, which is why this is not simply QEMU's
`adlib` with a better core. Address and data are the chip's two register
banks (bank 0 is the OPL2-compatible one). Reads return the status
register, where the two timers show. Every AdLib driver detects the chip
by writing timer 1, waiting and reading the flag back, so a chip whose
timers do not tick is one no game finds. The voice opens at the chip's
own 49716 Hz (`freq=`) and QEMU's mixer does the one conversion.
...
audio core's mixer-input registry, which the chip attaches to at realize.
Windows' Volume Control "MIDI" slider therefore works on FM music, and a
bare AdLib (no card, no mixer) plays at unity. CD audio is the other
input, under the card's CD volume and output switch. Upstream QEMU
stored these registers and applied none.
...
hardware does, and that is how a game decides the port exists. There is
no MIDI *in*, so the only byte the guest ever reads is that ACK.
...
The user sees **Windows 98 reboot on its own**. The reproduction is Duke
Nukem 3D's `SETUP.EXE` in a DOS box: Choose Music Card, General Midi,
0x330, Test Music Card.
...
machine could never catch this, because DOS leaves IRQ 9 masked.
...
the DSP status port is read, and QEMU models that by clearing bit 0 or 1
of mixer register 0x82 on a read of 0x2xE / 0x2xF. An assertion with
**no bit set** therefore holds the line for good. On the edge-triggered
ISA PIC every later completion is then a level 1 into an already-high
line, and the card stays deaf until the next DSP reset, with no error.
The two sites:
...
- **`reset()` pulsed the line while auto-init DMA ran**, an interrupt
  ...
  masked, so the edge sat in the master PIC's IRR with no owner, and
  Windows' VPICD will not unmask a physical IRQ in that state.
...
operation, the next start asks whether to skip DirectSound, and
DirectSound games do not run. It was first reported as "SB16 DirectSound
crashes on Linux and works on the Mac". The cause is **the guest's
language**, not the host, the card or QEMU: the same fault happens on
`embed`, on `none` and on a bare `qemu-system-i386`.

The fault is `exceção c0000409H no módulo DSOUND.DLL`, which is
`STATUS_STACK_BUFFER_OVERRUN` from the `/GS` cookie check in DirectX
9.0c's DSOUND.DLL (4.09.0000.0904). For `waveOutGetDevCapsA` and
`waveInGetDevCapsA`, the function copies a device's `szPname` with a
byte loop into a 32-byte buffer with the cookie behind it. A caps name is
a fixed 32 bytes and nothing requires a NUL. `TESTS\WAVECAPS.EXE` shows
it:
...
**The VxD has a way to override the name.** At start it reads
`WaveInDevName`, `WaveOutDevName` (and the MIDI, mixer, aux and
DirectSound names) from
...
Then it asks for the restart the VxD needs. A machine whose names fit is
told "nothing to do". On a copy of `claude98`, after the restart,
WAVECAPS read `"Entrada de som da SB16 [220]" nul_at=28` and dxdiag
stayed up. `tools/setup-guest-test.sh` requires the component's line on
Win98.

## 6. What a machine offers

The machine form (doc 07) has two pickers, both `launcher-core`'s
(ADR-014). They follow the display-adapter picker's rule: a family
...
add no drivers to. Both have the port one pick away, for example a real
MT-32 for an old game under XP.
...
removed). Windows 98 has no ES1370 driver in its box, and the AudioPCI
packages tried by hand did not make the card work. A second card for
Windows would have to start from a driver known to bind under our QEMU.
...
`docs/testing.md` describes how to run each one.
...
Check these first for a silent game. They separate a game that never
wrote to the port (its setup names another device, or it found nothing)
from one writing to a port that does not play. Duke Nukem 3D's *Sound
Blaster* music entry refuses to initialise and writes nothing. Its
*AdLib* entry, which probes 0x388, fills the log.
...
- **`play`** renders the capture through the same engine at its original
  timing. If the music breaks there, the bug is ours and the capture is
  the reproduction. If not, look at the guest.

Both devices capture the same way on purpose. **When a guest's two
hardware synthesizers misbehave together and its software one does not,
the engines are not the suspect**, because they share no synthesis code.

## 8. Not here (and the order to add it)

1. **A host MIDI port**, sending the stream to real hardware or the
   host's synth (CoreMIDI / ALSA / WinMM through `midir`). It needs
   player-side code, an embed API bump for the byte sink, and a port
   list in the machine form. This is the "real MIDI" case for someone
   who owns a module.
2. **MIDI in.** Nothing of the era needs it. The IRQ and the status
   register's second flag exist for it, and adding it is a queue and a
   `qemu_irq`.
