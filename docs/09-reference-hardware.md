...
and the ATAPI traces are still to be taken. The rig stays stock, because
it is an oracle, not a dev machine.
...
recalled from the model's class, not read off the monitor, and
everything derived scales with them. Measure the picture width and check
the pitch in the manual before treating them as fixtures. The photo set
below turns the approximation into a calibration.
...
- **CD-ROM (docs 05, 17).** Golden ATAPI traces as fixtures for the
  virtual drive: the MMC command, response and sense sequences a
  protected title (SafeDisc, SecuROM) issues during its disc check,
  captured on the rig or the same drive in a modern box. Known-good dumps
  with subchannel and error data from discs that pass on the rig. And the
  A/B: a title that fails in the VM but passes on the rig is a backend bug
  by definition.
...
primary, with no blit and no stretch, because otherwise the photo would
measure a scaler. `build/crtcal-render` (`tools/crtcal-render.c`, the
...
`.COM`, run from FreeDOS or a "Restart in MS-DOS mode" screen. It
...
- **Pattern 2, the 9th column.** A cell is 9 pixels wide and a glyph 8;
  ...
  stripes on one half and continuous white on the other. That is doc 03
  rule 2's "9-dot characters", shown.
...
Checked under our own emulator with FreeDOS running `TEXTCAL.COM` from
`FDAUTO.BAT`. A QMP screendump reports **720x400**, and the player logs
...
- **RAW if available**, and record the OSD settings (brightness, contrast,
  colour-temperature preset). Turn **moiré reduction off**, since it
  defocuses the beam on purpose.
