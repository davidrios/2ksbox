...
The package runs on the user's PC (Ryzen 9 5900X, RTX 3090): Win98
guests with the Voodoo 2, dxdiag and 3DMark 99 on DXVK, D3D7TEST on both
...
the install layout are doc 07's.
...
holds both builds side by side. The *sources* are shared, though: never
...
- **libslirp from source** — there is no `mingw64-libslirp`, and
  `-netdev user`, which every launcher machine asks for, is compiled in.
...
- clang and lld, with `packaging/windows/clang-mingw-cc` / `-cxx` as the
  compiler wrappers: QEMU is built with clang (patch 68), because mingw
...
the top, every DLL beside them (where the loader looks, so no rpath and
no `PATH`), data directories under it.
...
double-click opens a black terminal for the session. That costs three
things, all paid:
...
- somewhere for a failure to go — `launcher-core/src/fatal.rs`, the
  launcher's first call: `launcher.log` beside `player.log`, a milestone
  per start-up step (the last line names the step that died), and a
  panic hook that files message, location and backtrace and says so in a
...
outside the checkout with an empty environment: the launcher's `--paths`
must answer inside the package; the player's `--companions` must name
the staged executor and `dxvk_d3d9.dll` (loaded by name, in no import
table); the display driver's host test must draw through that pair and
read the right pixels (through winevulkan; skipped without a Vulkan
device); and the packaged `qemu-img.exe` must write a qcow2, which also
proves the DLL closure. Wine is not the target — a failure there is
investigated, not believed — but a package that fails these is broken
for every Windows.

## Which Direct3D 9 the executor runs on

DXVK, as everywhere (ADR-007) — and on Windows only, the **system's own
Direct3D 9** as the fallback for a host below DXVK's Vulkan 1.3 floor
(ADR-007's second amendment, 2026-09-21). Pre-Broadwell Intel, Kepler
...
driver's records never present, so the swapchain, the scene and the
Present — what the system implementation is strictest about — are only
...
it is WGL with a `WGL_ARB_pbuffer` standing in for the window — closer
to Linux than macOS is, since Windows has a real offscreen drawable. It
runs: `TESTS\GLPROBE.EXE` in a Win98 guest on the user's PC reads
...
`2ksbox.exe` is `launcher-qt` (ADR-015), so the zip carries Qt: the
DLLs, the platform plugin and the QtQuick QML trees. Fedora ships
...
- `CXX_QT_AUTORCC_OPTIONS=--no-zstd` (set in the image): the host `rcc`
  has zstd and the mingw `Qt6Core` does not, so a default resource does
  not link.
- **There is no cross `windeployqt`**, so `package-windows.sh` deploys
  Qt by hand: the plugin directories (**no window without
...
Platform), and the build has it. The launcher says so rather than
saying KVM: `Accel::Auto` is `whpx:tcg`, "hardware acceleration
...
- **Live control** — the launcher's QMP socket for snapshots and the
  disc shelf — is a Unix-domain socket on Windows too: QEMU binds
  `unix:`, the launcher connects through Winsock AF_UNIX
...
`scripts/win-run.sh` does what a package does by being one folder:
`build/win/qemu` on `PATH` for the embed DLL, the player named to the
launcher, and the executor and DXVK named to QEMU (DXVK copied to
`dxvk_d3d9.dll`). The launcher writes nothing to the terminal; paste the
...
- **lld is named through meson's `CC_LD`** rather than
  `clang-mingw-cc`: a native meson cannot run a shell script as a
...
which sources `guest-tools/msys2-i686.sh` first — the whole port:
...
- **It links Linux's i686 runtime, not MSYS2's.** MSYS2 builds its
  ...
  them with `-B`/`-L`, headers stay MSYS2's. MSYS2's i686 gcc must be
...
- Paths need no conversion: MSYS2 rewrites `/c/…` arguments and
  ...
  native program; Watcom's `@file` alone gets `cygpath -m`.
